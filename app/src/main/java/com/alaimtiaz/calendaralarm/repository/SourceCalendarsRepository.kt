...
class SourceCalendarsRepository(
     ...
     * @param windowDays — number of days ahead to sync (default 90).
     * @return list of events freshly written to Room.
     */
    suspend fun syncEnabledCalendars(windowDays: Int = 90): SyncResult {
        if (!hasCalendarPermission()) return SyncResult(0, 0)

        val enabled = sourceDao.getEnabled()
        if (enabled.isEmpty()) {
            // Drop any events from previously-enabled calendars now disabled
            // (handled separately when user toggles off; but ensure clean state)
            return SyncResult(0, 0)
        }

        val now = System.currentTimeMillis()
        val end = now + windowDays * 24L * 60L * 60L * 1000L

        var inserted = 0
        var updated = 0

        for (calendar in enabled) {
            val (writes, externalIds) = readAndStoreEventsFor(calendar, now, end)
            inserted += writes.first
            updated += writes.second
            // Delete events that no longer exist in this calendar's window
    ...
    private suspend fun readAndStoreEventsFor(
        ...
        // Use Instances table with content URI that includes time bounds —
        // this expands recurring events for us automatically.
        ...
        context.contentResolver.query(
            ...
            while (c.moveToNext()) {
                ...
                // For recurring events we get one row per instance — use eventId+begin as unique key
